...
//  public static final int CDSIM = 0; //XXX remove!
  ...
  /**
   * If present, this parameter defines the name of the experience, which is used by the FileObserver
   * abstract class to define the base file name for output files, when the option is defined. 
   * The default value is "Simulation-timestamp", where timestamp is the time at which the simulator
   * started.
   * 
   * @config
   */
  ...
  /**
   * If present, this parameter defines the data structure that is used for the Event Queue. The
   * default is the Peersim Heap. the value of the parameter must be the complete name of the 
   * class to be used to maintain the simulator events.
   */
  
  public static final String PAR_SIM_HEAP = "simulation.heap";
  ...
  public static void main(String[] args)
  {
    ...
    else
    {
      try
      {
        ...
        System.out.println("Experience terminated in " + duration + " miliseconds");
        System.out.println("Experience terminated in " + TimeUnit.MILLISECONDS.toHours(duration) + "h "
        											   + (TimeUnit.MILLISECONDS.toMinutes(duration) - TimeUnit.HOURS.toMinutes(TimeUnit.MICROSECONDS.toHours(duration))) + "m " 
        											   + (TimeUnit.MILLISECONDS.toSeconds(duration) - TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toSeconds(duration))) + "s");
      }
