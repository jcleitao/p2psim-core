 ...
 *
 * Changelog: Joao Leitao (May 2021): added the interface EventQueue which abstracts the interface of the event queue
 * 
...
public class Heap implements EventQueue
{
	...
	private final Events evs = new Events(1);
	...
	public Events removeMany( ) {
		if (size == 0)
			return null;
		evs.array[0].time = times[0];
		evs.array[0].event = events[0];
		evs.array[0].node = nodes[0];
		evs.array[0].pid = pids[0];
		evs.array[0].src = srcs[0];
		evs.size = 1;
		swap(1, size);
		size--;
		minHeapify(1);
	...
	public static void main(String[] args)
	{
		...
		if(errorCount > 0) {
			System.out.println("Incorrect order: " + errorCount + " unexpected values");
		}
